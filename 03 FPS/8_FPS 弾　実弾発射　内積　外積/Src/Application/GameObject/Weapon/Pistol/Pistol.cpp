...
void Pistol::Update()
{
	...
	if (m_shotFlg)
	{
		...
		if (isHit)
		{
			...
			{
				// レイの着地点を利用して弾を飛ばすベクトルを算出
				Math::Vector3 shotDir = hitPos - muzzlePos;

				// 発射
				std::shared_ptr<Bullet> bullet = std::make_shared<Bullet>();
				bullet->Init();
				bullet->Shot(muzzlePos, shotDir);
...
void Pistol::Init()
{
	if (!m_spModel)
	{
		...
		// 親から銃口へのローカル行列（相対位置）
		// ↑「銃本体から銃口への相対位置」と「親から向き本タオへの相対位置」を合成
		m_localMuzzleMat = Math::Matrix::CreateTranslation(0.0f, 0.18f, 0.7f);
